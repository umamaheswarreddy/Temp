 ...
 * @created-on 06/08/22
 */
public class StartandEndIndexexOfTarget {
    ...
    int bSearch(int[] arr, int target) {
        ...
        while (start <= end) {
            ...
            if (arr[mid] == target) {
                System.out.println(target+"start index of "+mid);
                for(int j=mid;j<end;j++) {
                    if (arr[mid] != arr[mid + 1]) {
                        System.out.println(target+"end index of "+j);
    ...
    public static void main(String[] args) {
        ...
        int target = 8;
        StartandEndIndexexOfTarget search = new StartandEndIndexexOfTarget();
